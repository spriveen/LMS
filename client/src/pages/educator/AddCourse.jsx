...
const AddCourse = () => {
  ...
  useEffect(() => {
    if (!quilRef.current && editRef.current) {
      quilRef.current = new Quill(editRef.current, {
        theme: 'snow',
      });
    }
  }, []);
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        ...
        <div>
          {chapter.map((ch, chapterIndex) => (
            <div key={ch.chapterId} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                <div className='flex items-center'>
                  <img
                    ...
                    width={14}
                    alt=''
                    className={`mr-2 cursor-pointer transition-all ${ch.collapsed && '-rotate-90'}`}
                  />
